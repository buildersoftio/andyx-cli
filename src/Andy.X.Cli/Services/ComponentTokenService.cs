...
{
    public static class ComponentTokenService
    {
        public static void GetComponentTokens(string tenant, string product, string component)
        {
            var node = NodeService.GetNode();

            string request = $"{node.NodeUrl}api/v3/tenants/{tenant}/products/{product}/components/{component}/tokens";
            try
            {
        ...
        public static void GetComponentToken(string tenant, string product, string component, Guid key)
        {
            var node = NodeService.GetNode();

            string request = $"{node.NodeUrl}api/v3/tenants/{tenant}/products/{product}/components/{component}/tokens/{key}";
            try
            {
                HttpClient client = new();
                client.DefaultRequestHeaders.Add("x-called-by", ApplicationParameters.ApplicationName);
                ...
                if (httpResponseMessage.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    var table = new ConsoleTable("KEY", "DESCRIPTION", "IS_ACTIVE", "SECRET", "ROLES", "ISSUED_FOR", "EXPIRE_DATE", "ISSUED_DATE");

                    var componentTokenDetails = JsonConvert.DeserializeObject<ComponentToken>(content);

                    if (componentTokenDetails!.IssuedFor == "")
                        componentTokenDetails.IssuedFor = "any";

                    table.AddRow(componentTokenDetails!.Id, componentTokenDetails.Description, componentTokenDetails.IsActive,
                        "********************",
                        string.Join(",", componentTokenDetails.Roles),
                        componentTokenDetails.IssuedFor,
                        componentTokenDetails.ExpireDate,
                        componentTokenDetails.IssuedDate);
                    table.Write();
                }
                else
                {
        ...
        public static void PostComponentToken(string tenant, string product, string component, ComponentToken componentToken)
        {
            ...
            try
            {
                ...
                if (httpResponseMessage.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    ...
                    Console.WriteLine("");
                    Console.WriteLine($"Component token has been created succesfully!");
        ...
        public static void RevokeComponentToken(string tenant, string product, string component, Guid key)
        {
            var node = NodeService.GetNode();

            string request = $"{node.NodeUrl}api/v3/tenants/{tenant}/products/{product}/components/{component}/tokens/{key}/revoke";
            try
            {
                HttpClient client = new();
                client.DefaultRequestHeaders.Add("x-called-by", ApplicationParameters.ApplicationName);
